...
<br>
 <form method="post">
            <table border="0" cellspacing="2" cellpadding="5">
                <thead>
                    <tr>
                        <th colspan="2" class="h2">Login Page</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
...
<center>
     ...
     <c:if test = "${not empty param.uname || not empty param.password}">
        ...
        <sql:query dataSource="${dbsource}" var="user1">
       	<c:set var= "uname1" value='${param.uname}'/>
        	<c:set var ="password1" value = '${param.password}'/>  
            SELECT count(*) FROM group_project.user where Username=? and Password= ?; 
           <sql:param value="${uname1}" />
            <sql:param value="${password1}" />       
        </sql:query>
       ...
       <c:choose>
         ...
         <c:set var="name" scope="application" value="${param.uname}"/>
     		<c:redirect url="AdminPage.jsp"/>
         </c:when>
         
         <c:when test = "${user1.rowsByIndex[0][0]==1}">
         <c:set var="name1" scope="application" value="${param.uname}"/>
